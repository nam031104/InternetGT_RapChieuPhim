document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderDateButtons() {
    ...
    allShowTimes.forEach((showtime) => {
      ...
      button.addEventListener("click", () =>
        handleDateSelect(showtime.date, showtime.label, button)
      );
  ...
  function updateSummary() {
    ...
    selectedSeats = Array.from(currentlySelectedSeats).map(
  ...
  function handleBook() {
    ...
    const confirmation = window.confirm(
      `Xác nhận đặt ${selectedSeats.length} ghế (${selectedSeats.join(
        ", "
      )}) cho suất ${selectedTime}, ${selectedDateLabel} với tổng số tiền ${
    ...
    if (confirmation) {
      console.log(
        "Đã đặt vé thành công! (Chuyển sang xử lý thanh toán thực tế...)"
      );
